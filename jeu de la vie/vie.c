...
void colorier_surface(int mat[50][50],int x,int y ,int w,int h,SDL_Renderer* renderer)
{
   ...
   while(i<50)
  {
      ...
      y=y+20; // passer à le deuxième ligne de la matrice
      j=0; 
      x=0;
    ...
    while(!go)
     {
       colorier_surface(mat,x,y,w,h,renderer);
        while(SDL_PollEvent(&event))
        {
          switch(event.type)
          {
            case SDL_MOUSEBUTTONDOWN  :
                k = event.button.x /20 ; //calcul des coordonnées de la case
                l = event.button.y /20 ; //On divise par 20 car chaque case est 20pixel x 20pixel
            ...
            case SDL_KEYDOWN :
                if(event.key.keysym.sym == SDLK_KP_ENTER) 
                 {
                    go = SDL_TRUE ; //La touche Entrée termine cette boucle et déclanche le jeu
                    fait = 1 ;
                     break ;
                 }
          }
        ...
        {
            colorier_surface(mat,x,y,w,h,renderer) ;
            printf("le jeu va commencé dans 4 secondes \nSoyez chaud!\n");
            SDL_Delay(4000);
